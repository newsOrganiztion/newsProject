...
const ArticleManagement = () => {
  ...
  const updateArticleStatus = async (id, newStatus) => {
    try {
      await axios.patch(`http://localhost:5000/api/article/${id}/status`, {
        status: newStatus,
      }, {
        headers: {
  ...
  const filteredArticles = articles.filter(article => {
    const matchesSearch = article.title && article.title.includes(searchTerm) || 
  ...
  return (
    <div className="w-full p-2 sm:p-4 md:p-6 bg-[#f9f9fb] min-h-screen" dir="rtl">
      ...
      {/* Filters Section */}
      <div className={`bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-md mb-4 sm:mb-6 flex flex-col md:flex-row gap-4 ${showFilters ? 'block' : 'hidden md:flex'}`}>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#f3f4ff]">
                  <tr>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">العنوان</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">الكاتب</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">الفئة</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">التاريخ</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">الحالة</th>
                    <th className="px-4 py-3 text-right text-sm font-medium text-[#51a31d]">الإجراءات</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredArticles.map((article) => (
                    <tr key={article._id} className="hover:bg-[#f9feff] transition-colors duration-200">
  ...
  <div className="text-xs text-gray-500 mt-1 h-10 overflow-y-auto">
    {article.description || "لا يوجد ملخص للمقال"}
                      ...
                      <td className="px-4 py-3 text-gray-900 font-medium">{article.author}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                          article.status === "published" ? "bg-[#51a31d]" : 
                          article.status === "pending approval" ? "bg-[#e4a11b]" : 
                          "bg-[#d94e5c]"
                        } text-white`}>
                          {article.status === "published" ? "منشور" : 
                          article.status === "pending approval" ? "بانتظار الموافقة" : 
                          "مسودة"}
                        </span>
                      </td>
                      <td className="px-4 py-3">
            ...
            <div className="md:hidden">
              {filteredArticles.map((article) => (
                <div key={article._id} className="border-b border-gray-200 p-4 hover:bg-[#f9feff]">
                  ...
                  <div className="text-sm text-gray-700 mb-2">
                    <span className="font-medium">الكاتب: </span>{article.author}
                  </div>
