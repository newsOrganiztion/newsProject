...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put('http://localhost:5000/api/users/profile', updatedUser, {
        withCredentials: true
      });
      setUser(res.data.user);
      setIsEditing(false);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-xl text-[#383838]">
          <svg className="animate-spin -ml-1 mr-3 h-8 w-8 text-[#51a31d]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="bg-[#383838] border border-[#51a31d] text-white px-4 py-3 rounded-lg relative" role="alert">
  ...
  return (
    <div className="bg-[#f9f9fb] min-h-screen py-8 px-4 sm:px-6 lg:px-8" dir="rtl">
      <div className="max-w-4xl mx-auto">
        {updateSuccess && (
          <div className="bg-[#f9f9fb] border border-[#7585ff] text-[#383838] px-4 py-3 rounded-lg relative mb-4" role="alert">
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center">
              {user.profilePicture ? (
                ...
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-gray-300 flex items-center justify-center border-4 border-white shadow-md">
                  <span className="text-4xl text-gray-600">{user.name?.charAt(0).toUpperCase() || '؟'}</span>
                </div>
              )}
          ...
          <div className="p-6">
              ...
              {isEditing ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-[#f9f9fb] p-4 rounded-lg md:col-span-2">
                      <label className="text-sm text-gray-500 block mb-1">رابط الصورة الشخصية</label>
                      <input
                        type="url"
                        name="profilePicture"
                        ...
                        className="w-full p-2 border border-[#7585ff] rounded-md bg-[#f9f9fb]"
                      />
                      <small className="text-gray-500">اترك الحقل فارغًا إذا كنت لا تريد تغيير الصورة</small>
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <button type="submit" className="bg-[#7585ff] hover:bg-[#6a79e6] text-white px-6 py-2 rounded-lg transition-colors">
                      حفظ التغييرات
                    </button>
                  </div>
                </form>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#f9f9fb] p-4 rounded-lg">
                    ...
                    <p className="font-medium text-gray-800">{new Date(user.createdAt).toLocaleDateString('ar-EG')}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* المقالات المحفوظة */}
            <div className="border-b border-gray-200 pb-4 mb-4">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">المقالات المحفوظة</h2>
            ...
            {/* التعليقات */}
            <div className="border-b border-gray-200 pb-4 mb-4">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">التعليقات</h2>
            ...
            {/* تاريخ القراءة */}
            <div>
              <h2 className="text-xl font-semibold text-gray-800 mb-4">تاريخ القراءة</h2>
              {user.readingHistory && user.readingHistory.length > 0 ? (
                <div className="bg-[#f9f9fb] rounded-lg p-4">
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {user.readingHistory.map((history, index) => (
                      <li key={index} className="flex items-center bg-white p-3 rounded shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#7585ff] ml-2" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                        </svg><span className="text-gray-700">{history}</span>
                      </li>
                    ))}
                  </ul>
                </div>
