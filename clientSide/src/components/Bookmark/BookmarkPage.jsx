...
const BookmarkPage = () => {
  ...
  // Fetch user ID and saved articles
  useEffect(() => {
    ...
    const getSavedArticles = async () => {
      try {
        ...
        if (response.data.savedArticles) {
          setBookmarks(response.data.savedArticles);
          setHasBookmarks(response.data.savedArticles.length > 0);
        } else {
          console.error('لم يتم العثور على إشارات مرجعية');
        }
      } catch (error) {
  ...
  // Function to handle card click (if needed)
  const handleCardClick = (bookmarkId) => {
    console.log('تم النقر على المقال: ', bookmarkId);
    // Add additional behavior here if required
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-[#51a31d]/5" dir="rtl">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
              ...
              <div className="flex space-x-3 space-x-reverse self-end sm:self-auto">
                <button
                  ...
                  className={`flex items-center space-x-2 space-x-reverse px-4 py-2 rounded-lg border transition-all duration-500 ${viewMode === 'grid' ? 'bg-[#51a31d] text-white border-[#51a31d] shadow-lg shadow-[#51a31d]/20 scale-105' : 'bg-white text-[#383838] border-gray-200 hover:border-[#51a31d] hover:text-[#51a31d]'}`}
                >
                  <LayoutGrid size={18} />
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 space-x-reverse px-4 py-2 rounded-lg border transition-all duration-500 ${viewMode === 'list' ? 'bg-[#51a31d] text-white border-[#51a31d] shadow-lg shadow-[#51a31d]/20 scale-105' : 'bg-white text-[#383838] border-gray-200 hover:border-[#51a31d] hover:text-[#51a31d]'}`}
                >
                  <List size={18} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {hasBookmarks ? (
          <div className={`${viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8' : 'flex flex-col space-y-6'} opacity-0 transform translate-y-8 transition-all duration-1000 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : ''}`}>
            {bookmarks.map((bookmark, index) => (
              <div
                key={bookmark._id}
                onClick={() => handleCardClick(bookmark._id)}
                className={`group bg-white rounded-lg overflow-hidden transition-all duration-500 ${viewMode === 'list' ? 'flex flex-row-reverse' : 'flex flex-col'} opacity-0 transform translate-y-8 ${isLoaded ? 'opacity-100 translate-y-0' : ''} transition-all duration-700 ease-out border border-gray-100 hover:border-[#51a31d] shadow-md hover:shadow-lg relative`}
              ...
              >
                ...
                {/* Article Image */}
                <div className={`${viewMode === 'list' ? 'flex-shrink-0 w-36 sm:w-48' : 'h-48'} relative overflow-hidden z-10`}>
                  <img
                    src={`http://localhost:5000/${bookmark.featuredImage}`}
                    alt={bookmark.title}
                    className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#383838]/70 via-transparent to-transparent opacity-40 transition-opacity duration-300"></div>
                  <div className="absolute -top-2 -right-2 bg-white/80 backdrop-blur-sm text-[#383838] px-3 py-1 rounded-md text-xs font-medium shadow-sm">
                ...
                <div className="p-5 flex-grow flex flex-col relative z-10">
                  ...
                  <p className="text-sm text-[#383838]/80 mb-4 line-clamp-2">
                    {bookmark.description || "وصف مختصر للمقال يظهر هنا."}
                  </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-10 text-center transition-all duration-700 transform opacity-0 translate-y-8 animate-fadeIn">
            ...
            <p className="text-[#383838]/70 max-w-lg mx-auto mb-10">
              ستظهر المقالات التي قمت بحفظها هنا.
            </p>
          </div>
        )}
      </main>
